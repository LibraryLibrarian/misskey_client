...
여러 항목을 변경하는 헬퍼는 변경을 시작한 뒤 개별 작업 실패를 예외로 던지지 않고, 입력 순서대로 입력마다 하나의 결과를 담은 `MisskeyBatchResult<I, T>`에 기록합니다. 결과가 확정된 항목을 보고하는 `onProgress` 콜백에서 사용자가 던진 오류는 다릅니다. 새 작업을 중단하고 진행 중 요청이 끝난 뒤 그 오류를 다시 던지며, 이미 완료된 변경은 롤백하지 않습니다.
