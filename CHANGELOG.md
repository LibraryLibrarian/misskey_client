...
- **Breaking:** parameters changed to `Optional<T>` by the null-clearing fix below no longer accept a bare value. Wrap it: `description: 'text'` becomes `description: Optional('text')`. Affected so far: `ClipsApi.update()`, `ChannelsApi.update()`, `PagesApi.update()`, `AdminAnnouncementsApi.update()`, `AdminApi.updateProxyAccount()`, `AntennasApi.update()`, `DriveFilesApi.update()`, `GalleryApi.postsUpdate()`, `AdminAvatarDecorationsApi.update()`
